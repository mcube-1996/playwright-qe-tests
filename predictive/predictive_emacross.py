...
class EMA_Cross_Printer(bt.Strategy):
    ...
    def __init__(self):
        ...
        # Track crossovers: +1 when slow crosses above fast, -1 when slow crosses below fast.
    ...
    def next(self):
        # 200 EMA (slow) crosses above 50 EMA (fast): record entry price
        if self.cross[0] == 1:
            # Only record entry if not already in a trade
            if self.entry_price is None:
                self.entry_price = self.data.close[0]
                print(f"Entry Price: {self.entry_price}")

        # 200 EMA crosses below 50 EMA: record exit price and evaluate trade
        elif self.cross[0] == -1:
            if self.entry_price is not None:
                ...
                # Reset the entry price after closing the trade
                self.entry_price = None
